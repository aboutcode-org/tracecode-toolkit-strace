...
from tracecode.tracecode import (Process, Exec, Readwrite)  # @UnusedImport
from tracecode.tracecode import as_graph


ROOT_DIR = unicode(dirname(dirname(os.path.abspath(__file__))))
...
class BaseTestCase(unittest.TestCase):
    ...
    def extract_trace(self, base, delete=True):
        ...
        """
        input_tar = join(self.get_tst_path(base),
                                 'trace-archive.tar.bz2')
    ...
    def failUnlessEqual(self, first, second, msg=None):
        """
        Improved failure reporting for strings and list comparisons.
        """
        def _as_list(it):
            if type(it) == types.GeneratorType:
                return [x for x in it]
            else:
                return it

        first = _as_list(first)
        second = _as_list(second)
        if (type(first) in (list, tuple, set, str, unicode, basestring,)
            and type(first) == type(second)):
            try:
                unittest.TestCase.failUnlessEqual(self, first, second)
            except AssertionError:
                if (type(first) in [list, tuple, set]
                    and type(first) == type(second)):
                    diffs = difflib.ndiff([str(e) for e in first],
                                          [str(e) for e in second])
                else:
                    diffs = difflib.ndiff(repr(first).split('\n'),
                                          repr(second).split('\n'))
                raise AssertionError, ('Differences found '
                                       '(- = first, + = second)\n'
                                       + '\n'.join(diffs))
        else:
    ...
    def check_pformats(self, test_name, cwd, with_output_dir=False,
                       clean=False, parallel=False, regen=False,
                       settings=None):
        """
        ...
        procs = tracecode.parse_raw_traces(cwd, input_dir, output_dir,
                                           parallel=parallel,
                                           settings=stgs, clean=clean)
        if with_output_dir:
            procs = tracecode.load(output_dir, procs)

        if regen or _GLOBAL_REGEN:
            self.regen_pformats(test_name, procs)
        ar = self.get_tst_path(join(test_name,
                                            'expected_pformats.tar.bz2'))
        expected_pf = join(self.extract_archive(ar),
                                   'expected_pformats')
        for proc in procs:
...
class BasicTest(BaseTestCase):
    ...
    def test_parse_entry(self):
        expected = [
            tracecode.Entry(tstamp='1389171522.375781', result='0', func='execve', args=['/usr/bin/make', 'make']),
            tracecode.Entry(tstamp='1389171522.376446', result='4', func='open', args=["/lib/x86_64-linux-gnu/libc.so.6", 'O_RDONLY|O_CLOEXEC']),
            tracecode.Entry(tstamp='1389171522.376468', result='832', func='read', args=['4</lib/x86_64-linux-gnu/libc-2.15.so>', "\\177ELF\\2\\1\\1...", '832']),
            tracecode.Entry(tstamp='1389171522.376782', result='0', func='arch_prctl', args=['ARCH_SET_FS', '0x7ff784398700']),
            tracecode.Entry(tstamp='1389171522.377455', result='35', func='getcwd', args=["/home/nexb/tools/strace/strace-4.8", '4096']),
            tracecode.Entry(tstamp='1389171522.377657', result='4', func='openat', args=['AT_FDCWD', ".", 'O_RDONLY|O_NONBLOCK|O_DIRECTORY|O_CLOEXEC']),
            tracecode.Entry(tstamp='1389171522.377780', result='4', func='open', args=["Makefile", 'O_RDONLY']),
            tracecode.Entry(tstamp='1389171522.392265', result='4', func='openat', args=['AT_FDCWD', "linux/x86_64", 'O_RDONLY|O_NONBLOCK|O_DIRECTORY|O_CLOEXEC']),
            tracecode.Entry(tstamp='1389171522.392467', result='22', func='write', args=['1</dev/pts/6>', '/bin/mkdir -p ./linux\\n', '22']),
            tracecode.Entry(tstamp='1389171522.392584', result='2453403', func='vfork', args=[]),
            tracecode.Entry(tstamp='1389171528.651642', result='0', func='chdir', args=['/home/nexb/tools/strace/strace-4.8']),
            tracecode.Entry(tstamp='1390356496.673733', result='0', func='execve', args=['/usr/lib/gcc/x86_64-linux-gnu/4.6/cc1', '/usr/lib/gcc/x86_64-linux-gnu/4....', '-quiet', '-I', '.', '-I', '.', '-I', './include', '-I', './lib', '-imultilib', '.', '-imultiarch', 'x86_64-linux-gnu', '-D', 'PROGRAM=\"bash\"', '...']),
        ]
    ...
    def test_parse_entry_dup(self):
        ...
        expected = tracecode.Entry(tstamp='1390356525.416370', result='6',
                                   func='dup', args=['4<pipe:[8091470]>'])
        result = tracecode.parse_entry(line)
        assert expected == result

    def test_parse_entry_dup2(self):
        line = '1390356525.416370 dup2(9<pipe:[8060]>, 1<pipe:[8058]>) = 1'
        expected = tracecode.Entry(tstamp='1390356525.416370',
                                   result='1',
                                   func='dup2',
                                   args=['9<pipe:[8060]>',
                                         '1<pipe:[8058]>'])
    ...
    def test_parse_entry_resolve_descriptor_with_deleted_descriptor(self):
        ...
        expected = tracecode.Entry(tstamp='1390356525.416370', result='61', func='read', args=['0</tmp/sh-thd-1391680596>', '#include <libintl.h>\nint main( deleted)\n...', '61'])
        result = tracecode.parse_entry(line)
        assert repr(expected) == repr(result)
        tracecode.resolve_descriptors(result)
        expected2 = tracecode.Entry(tstamp='1390356525.416370', result='61', func='read', args=['/tmp/sh-thd-1391680596', "#include <libintl.h>\nint main( deleted)\n...", '61'])
        assert expected2 == result

    def test_parse_entry_with_space_in_paths(self):
        expected = [
            tracecode.Entry(tstamp='1389171522.375781', result='0', func='execve', args=['/usr/bin/ma ke', 'make']),
            tracecode.Entry(tstamp='1389171522.376446', result='4', func='open', args=["/lib/x86_64-linux  -gnu/libc.so.6", 'O_RDONLY|O_CLOEXEC']),
            tracecode.Entry(tstamp='1389171522.376468', result='832', func='read', args=['4</lib/x86_64-linux  -gnu/libc-2.15.so>', "\\177ELF\\2\\1\\1...", '832']),
            tracecode.Entry(tstamp='1389171522.376782', result='0', func='arch_prctl', args=['ARCH_SET_FS', '0x7ff784398700']),
            tracecode.Entry(tstamp='1389171522.377455', result='35', func='getcwd', args=["/home/nexb/tools/str  ace/strace-4.8", '4096']),
            tracecode.Entry(tstamp='1389171522.377657', result='4', func='openat', args=['AT_FDCWD', "some path", 'O_RDONLY|O_NONBLOCK|O_DIRECTORY|O_CLOEXEC']),
            tracecode.Entry(tstamp='1389171522.377780', result='4', func='open', args=["Makefile", 'O_RDONLY']),
            tracecode.Entry(tstamp='1389171522.392265', result='4', func='openat', args=['AT_FDCWD', "lin  ux/x86_64", 'O_RDONLY|O_NONBLOCK|O_DIRECTORY|O_CLOEXEC']),
            tracecode.Entry(tstamp='1389171522.392467', result='22', func='write', args=['1</dev/p  ts/6>', '/bin/mkdir -p ./linux\\n', '22']),
            tracecode.Entry(tstamp='1389171522.392584', result='2453403', func='vfork', args=[]), tracecode.Entry(tstamp='1389171528.651642', result='0', func='chdir', args=['/home/nexb/tools/str  ace/strace-4.8']),
            tracecode.Entry(tstamp='1390356496.673733', result='0', func='execve', args=['/usr/lib/gcc/x86  _64-linux-gnu/4.6/cc1', '/usr/lib/gcc/x86_64-linux-gnu/4....', '-quiet', '-I', '.', '-I', '.', '-I', './include', '-I', './lib', '-imultilib', '.', '-imultiarch', 'x86_64-linux-gnu', '-D', 'PROGRAM=\"bash\"', '...']),
        ]
    ...
    '''1389171522.418917 open("/usr/share/locale/en_US.UTF-8/LC_MESSAGES/gcc-4.6.mo", O_RDONLY) = -1 ENOENT (No such file or directory)
    ...
    ['4</lib/x86_64-linux-gnu/libtinfo.so.5.9>', '\\177ELF\\2\\1\\310-"\\0\\0\\0\\0\\0\\310-"\\0\\0\\0\\0\\0X8\\0\\0...', '832']),
    (r'''4</tmp/ccGJ3nlX.s>, "ue\t0x1\n\t.byte\t0x55\n\t.quad\t.LVL10-.Ltext0\n\t.value\t0x1\n"..., 4096''',
    ['4</tmp/ccGJ3nlX.s>', 'ue\\t0x1\\n\\t.byte\\t0x55\\n\\t.quad\\t.LVL10-.Ltext0\\n\\t.value\\t0x1\\n...', '4096']),
    (r'''4</tmp/ccGJ3nlX.s>, "g\t\"u_error\"\t\"doth\"\n.\"short int\"\n\"m\"\n.LASF7:\"__off_t\"\n.LA"..., 993''',
    ['4</tmp/ccGJ3nlX.s>', 'g\\t"u_error"\\t"doth"\\n."short int"\\n"m"\\n.LASF7:"__off_t"\\n.LA...', '993']),
    ...
    ['4</tmp/cc  GJ3nlX.s>', 'ue\\t0x1\\n\\t.byte\\t0x55\\n\\t.quad\\t.LVL10-.Ltext0\\n\\t.value\\t0x1\\n...', '4096']),
    (r'''4</tmp/cc  GJ3nlX.s>, "g\t\"u_error\"\t\"doth\"\n.\"short int\"\n\"m\"\n.LASF7:\"__off_t\"\n.LA"..., 993''',
    ['4</tmp/cc  GJ3nlX.s>', 'g\\t"u_error"\\t"doth"\\n."short int"\\n"m"\\n.LASF7:"__off_t"\\n.LA...', '993']),
    ...
    ['4</cgi-bin/websearch>', '\\327\\362\\256\\200;<H\\367\\321L\\215d\\n\\376u\\31A\\200<$>t\\nH\\307E\\0\\0\\0\\0\\0\\353...', '4096']),
                ]
        for args, expected in tests:
            dec = tracecode.decode_args(args)
            assert expected == dec
    ...
    def test_decode_descriptor(self):
        tests = [('4</lib/x86_64-linux-gnu/libtinfo.so.5.9>',
    ...
    def test_resolve_paths_with_rename(self):
        cwd = '/TEST/'
        entry = '1389098994.521748 rename("./net/core/.filter.o.tmp", "/net/core/.filter.o.cmd") = 0'
        e = tracecode.parse_entry(entry)
        er = tracecode.resolve_paths(e, cwd)
    ...
    def test_parse_line_new_process_clone(self):
        MockProcess = collections.namedtuple('Process',
                                             'ppid pid cwd children execs')
        cwd = '/'
        ex = tracecode.Exec(command='bin/ls', args=[], tstamp='0.1')
        proc = MockProcess(None, 1, cwd, {}, [tracecode.Exec(command='bin/bash', args=[], tstamp='0.1'), ex])
    ...
    def test_resolve_descriptors_fchdir(self):
        ...
        e = tracecode.parse_entry(entry)
        tracecode.resolve_descriptors(e)
        assert '/home/nexb/build/jb4.3-14.10.0Q3.X-79/frameworks/base/media/tests/MediaDump/res' == e.args[0]

    def test_resolve_descriptor_logs_warning_for_non_decodable_desc(self):
        entry = '''1389171522.388349 dup2(12</home/nexb/tools/strace/strace-4.8/Makefile>, 255) = 0'''
        e = tracecode.parse_entry(entry)
    ...
    def test_parse_line_exec_multiple(self):
        ...
        expected = ["../../../../../../kernel/scripts...", "arm-eabi-gcc",
                    "/home/nexb/build/jb4.3-14.10.0Q3...", "-I../../../../../../kernel/arch/...",
                    "-Inet/core", "-D__KERNEL__", "-mlittle-endian", "..."]
    ...
    def test_process_simple_command1(self):
        ...
        output_dir = None
        done = tracecode.parse_raw_traces(cwd, input_dir, output_dir,
                                          parallel=False,
    ...
    def test_process_simple_command2(self):
        ...
        preadwrites = [(r.source, r.target,) for r in proc.readwrites]
    ...
    def test_Process_object_can_be_pickled(self):
        proc = tracecode.Process(pid=1, ppid=0, cwd='/',
                                 init_exec='bash', tstamp='0.1')
        proc.children = {12: 13}
        proc.execs = [22, 23]
        proc.reads = ['a', 'b']
        proc.writes = ['c', 'd']
    ...
    def run_tracecode_command(self, args):
        script = on_windows and 'tracecode.bat' or 'tracecode'
        script = join(ROOT_DIR, script)
        return subprocess.call([script] + args, cwd=ROOT_DIR)
    ...
    def test_command_line_parse_simple(self):
        ...
        output_dir = self.get_temp_dir()
        args = ['-q', '--cwd=%s' % cwd, 'parse', input_dir, output_dir]
        ret = self.run_tracecode_command(args)
        assert ret == 0, ret
    ...
    def test_command_line_parse_with_ignored_reads(self):
        ...
        cwd = '/home/nexb/tools/strace/strace-4.8'
        output_dir = self.get_temp_dir()
        args = ['-q', '--cwd=%s' % cwd, '--ignored_reads="/blabbla/*"', 'parse', input_dir, output_dir]
        ret = self.run_tracecode_command(args)
        assert ret == 0, ret
    ...
    def test_is_multiplexed(self):
        ...
        proc = tracecode.Process(pid=1, ppid=0, cwd='/',)
        tracecode.parse_trace_file(proc, trace_file, None, done,
                                   output_dir=None)
    ...
    def test_demux(self):
        ...
        tracecode.parse_trace_file(proc, trace_file, None, done,
                                   output_dir=None, settings=bare_settings)
        stgs = conf.BaseSettings()
        muxers = stgs.multiplexers
        assert proc.is_multiplexed(muxers)
 ...
 Children:'''
    ...
    def test_demux_single_readwrite(self):
        proc = tracecode.Process(pid=1, ppid=0, cwd='/',
                                 init_exec=tracecode.Exec('/bin/cp', '', 0))
        muxers = ['*/cp']
        assert proc.is_multiplexed(muxers)
 ...
 Children:'''.splitlines()
    ...
    def test_filter_ignored(self):
        proc = tracecode.Process(pid=1, ppid=0, cwd='/', init_exec=tracecode.Exec('/bin/bash', '', 0))
        proc.add_read('/read/path', '0', [])
        proc.add_write('/write/path2', '0', [])
        ...
        proc.filter(ignored_reads=('*/path',), ignored_writes=('*/path*', '-*/path2',), ignored_execs=[])
    ...
    def test_filter_ignored_pipes_and_sockets(self):
        ...
        assert {} != proc.writes
        assert [] != proc.readwrites
        proc.filter(ignored_reads=('pipe:*', 'socket:*',), ignored_writes=('pipe:*', 'socket:*',), ignored_execs=[])
        assert set([]) == proc.reads_paths()
        assert set([]) == proc.writes_paths()

    def test_filter_ignored_execs(self):
        proc = tracecode.Process(pid=1, ppid=0, cwd='/',
                                 init_exec=tracecode.Exec('/bin/bash', '', 0))
        proc.add_read('/read/path', '0', [])
        proc.add_write('/write/path2', '0', [])
        proc.add_write('/write/path', '0', [])
        proc.add_readwrite('/read/read', '/write/read', '0', '0', [], [])
    ...
    def test_as_ops(self):
        ...
        output_dir = None
                                        ...
                                        settings=conf.DefaultSettings(),
                                        clean=False)
        expected = [tracecode.Operation(
            pid=2799712,
            command='/bin/grep',
            sources=['/home/nexb/tools/patchelf/patchelf-0.5/tests/simple.c',
            ...
            targets=['/home/license', ],
            start_tstamp='1389356960.191213', end_tstamp='1389356960.220131'
            )
        ]
        proc = done[0]
        ao = list(proc.as_ops())
    ...
    def test_as_ops_handles_multiplexing_correctly(self):
        ...
        procs = tracecode.parse_raw_traces(None, input_dir, output_dir=None,
                                           parallel=False,
                                           settings=conf.BaseSettings(),
                                           clean=False)
    ...
    def test_stats_no_stat(self):
        ...
        output_dir = None
        procs = tracecode.parse_raw_traces(None, input_dir, output_dir, parallel=True, settings=conf.BaseSettings())
...
------------------'''
    ...
    def test_stats_stat(self):
        ...
        settings = conf.BaseSettings()
        settings.stats = True
        procs = tracecode.parse_raw_traces(None, input_dir, output_dir, parallel=True, settings=settings)
        expected = '''Parsing statistics
==================
Counters
--------
...
'''
    ...
    def test_incomplete_trace_parsing_does_not_hangs_but_timesout_todir(self):
        ...
        try:
            ...
            # is a trace file without any reference in the stack of PIDs
            input_dir = self.extract_trace('parse_does_not_hang')
            tracecode.parse_raw_traces(None, input_dir, output_dir,
                                       timeout_func=timeoutfunc)
        except Queue.Empty:
            # success
            pass
    ...
    def test_parse_line_without_error(self):
        ...
        with open(join(bd, 'trace.1'), 'rb') as trace:
            for line in trace:
...
class PathMatchingTest(BaseTestCase):
    ...
    def test_match_paths_two_matches(self):
        paths1 = ['a/b/c']
        paths2 = ['d/b/c', 'c/b/c']
        expected = [('a/b/c', 'd/b/c'), ('a/b/c', 'c/b/c'), ]
        self.check_match(paths1, paths2, expected)
    ...
    def test_match_paths_long(self):
        def get_list(path):
            ...
            return sorted(open(f, 'rb').read().splitlines())
        ...
        regen = False
        if regen or _GLOBAL_REGEN:
            with open(self.get_tst_path('match_path_long/expected.lst'), 'wb') as o:
...
class FilesetTest(BaseTestCase):
    ...
    def test_in_fileset_sources(self):
        sources = ('/home/nexb/tools/patchelf/patchelf-0.5/*',
                   '-/home/nexb/tools/patchelf/patchelf-0.5/src/patchelf',
                   '-/home/nexb/tools/patchelf/patchelf-0.5/src/patchelf.o',
                   )
        assert not tracecode.in_fileset('/home/nexb/tools/patchelf/patchelf-0.5/src/patchelf', sources)
...
class EndToEndTest(BaseTestCase):
    ...
    def test_trace_complete_build_strace_full(self):
        ...
        self.check_pformats(test_name, cwd, with_output_dir=False,
                            parallel=True, regen=False)

    def test_trace_complete_build_strace_saved_to_dir(self):
        cwd = '/home/nexb/tools/strace/strace-4.8'
        test_name = 'pformat_strace_saved'
        self.check_pformats(test_name, cwd, with_output_dir=True,
                            parallel=True, regen=False)
    ...
    def test_trace_cleaning_strace_full(self):
        ...
        self.check_pformats(test_name, cwd, with_output_dir=False,
                            clean=True, parallel=True, regen=False)
...
class GraphicTest(BaseTestCase):
    ...
    def test_render_graph_bare(self):
        ...
        stgs = conf.DefaultSettings()
        procs = tracecode.parse_raw_traces(cwd, input_dir, output_dir=None, parallel=False, settings=stgs, clean=False)
    ...
    def test_render_graph_patchelf_clean_parallel(self):
        ...
        self.check_render_graph_dot('render_graph_patchelf', procs,
                                    settings=stgs)
...
class ListTest(BaseTestCase):
    ...
    def test_reads_writes_paths(self):
        ...
        done = tracecode.parse_raw_traces(cwd, input_dir, output_dir, parallel=False, settings=stgs, clean=False)
        proc = done[0]
    ...
    def test_file_lists(self):
        ...
        procs = tracecode.parse_raw_traces(cwd, input_dir, output_dir, parallel=False, settings=stgs, clean=False)
    ...
    def test_guess_sources_and_targets_for_patchelf_with_default_settings(self):
        ...
        assert expected_tgts == list(targets)
    ...
    def test_guess_sources_and_targets_from_dir(self):
        ...
        extra_read_ignored = (('/usr/lib/gcc/*', '*/Makefile',)
...
class TraceFullGraphAnalysisTest(BaseTestCase):
    ...
    def test_file_sets_sources_and_targets_in_graph(self):
        ...
        stgs = conf.Settings(
            ...
            ignored_reads=tuple(),
            ignored_writes=tuple(),
            sources=('/home/nexb/tools/patchelf/patchelf-0.5/src/elf.h',
                     '/home/nexb/tools/patchelf/patchelf-0.5/src/patchelf.cc'),
            targets=('/home/nexb/tools/patchelf/patchelf-0.5/src/patchelf',)
                             )

        procs = tracecode.parse_raw_traces(None, input_dir, output_dir,
                                           parallel=False,
                                           settings=stgs,
                                           clean=True)
        ...
        expected_srcs = open(self.get_tst_path(
            'file_graph_analysis_patchelf/expected_srcs')).read().splitlines()
        assert sorted(expected_srcs) == sorted(list(srcs))

        expected_tgts = open(self.get_tst_path(
            'file_graph_analysis_patchelf/expected_tgts')).read().splitlines()
        assert sorted(expected_tgts) == sorted(list(tgts))

        expected_ints = open(self.get_tst_path(
            'file_graph_analysis_patchelf/expected_ints')).read().splitlines()
    ...
    def analyze_d2d(self, test_name, cwd, settings, expected=None, regen=False):
        ...
        if not expected:
            ...
            expected = [(None if not x else x, None if not y else y,)
                                                for x, y in expected]
        assert expected == d2d
    ...
    def test_analyze_full_graph_patchelf_d2d_with_default_settings(self):
        ...
        stgs.sources = ('/home/nexb/tools/patchelf/patchelf-0.5/src/elf.h',
                     '/home/nexb/tools/patchelf/patchelf-0.5/src/patchelf.cc',)
    ...
    def test_analyze_cups_d2d_with_default_settings(self):
        ...
        stgs.sources = self._get_file_set(test_name, 'sources')
        targets = set('''/home/nexb/tools/cups/cups-1.4.6/backend/libbackend.a
...
targets=somefile
  someotherfile
'''
        args = {u'sources': (u'one', u'two', u'-three*',),
                    u'targets': (u'somefile', u'someotherfile',)}
